...
def load_image(file_name):
    ...
    with gzip.open(file_path, 'rb') as f:
        images = np.frombuffer(f.read(), np.uint8, offset=16) # set offset=16 to skip metadata
    return images
...
def load_label(file_name):
    file_path = file_name
    with gzip.open(file_path, 'rb') as f:
        labels = np.frombuffer(f.read(), np.uint8, offset=8)  # set offset=8 to skip metadata
        
        
        one_hot_labels = np.zeros((labels.shape[0], 10)) # one_hot_labels is a 2D array
...
def load_mnist():
    ...
    dataset['x_train'] /= 255.0 # convert 8 bits integer to be a 0~1 float number
    dataset['x_test'] /= 255.0

    dataset['x_train'] = dataset['x_train'].reshape(-1, 28*28) # convert 1D array to 2D array
